...
import { ScrollView, View, Text, Image, TouchableOpacity, ListView, TextInput } from 'react-native';
...
class Permission extends Component {
    ...
    title: navigation.state.params && navigation.state.params.id ? '修改角色' : '创建角色',
    headerRight: (
      <Icon.Button
        name="md-checkmark"
        color={'#000'}
    ...
    initData() {
        ...
        const id = state.params && state.params.id || -1;
        if (id !== -1) {
            this.setState({
                name: '管理员',
            });
                ...
                <View style={[styles.orgWrapper, {marginLeft: -20, marginRight: -20,}]}>
                    <View style={[styles.orgList, {borderLeftWidth: 0,}]}>
                        <View style={styles.orgHeader}>
                            <Text style={{ color: '#fff' }}>{this.state.name ? this.state.name.substring(0, 1) : ''}</Text>
                        </View>
                        <View style={styles.orgSection}>
                            <TouchableOpacity
